...
def test_create_task(task_service: TaskService):
    task_in = TaskCreate(description="Test Task", input={"foo": "bar"})
...
def test_get_task(task_service: TaskService):
    task_in = TaskCreate(description="Retrieve Task")
...
def test_list_tasks(task_service: TaskService):
    for i in range(3):
        task_in = TaskCreate(description=f"Task {i}")
...
def test_complete_task(task_service: TaskService):
    task_in = TaskCreate(description="Complete Me")
    created_task = task_service.create_task(task_in)

    completed_task = task_service.complete_task(created_task.id, result="Done successfully")
    assert completed_task.status == TaskStatus.completed
    assert completed_task.result == "Done successfully"


def test_fail_task(task_service: TaskService):
    task_in = TaskCreate(description="Fail Me")
    created_task = task_service.create_task(task_in)

    failed_task = task_service.fail_task(created_task.id, error="Something went wrong")
    assert failed_task.status == TaskStatus.failed
    assert failed_task.result == "Something went wrong"
